...
- **canvas 简历复制提取（v1.4.0）**: iframe 内探测到 `#resume` 里有宽高正常的 `canvas` 且 `textContent` 不足阈值时：WASM 渲染（body 含 `wasm-resume-container`/`wasm-resume`）走 `tryExtractCanvasResumeByDragCopy()`，旧版 canvas 走 `tryExtractResumeTextByTrustedCopy()`。拖拽复制端点 `/canvas-copy`（cdp-proxy.mjs）模拟「鼠标选中 → 滚动容器 → 底部延伸选中 → 真实 Ctrl+C」，Boss 的 copy 处理器把全文写进**系统剪贴板**（页面级拦截看不到），调用方用 PowerShell `Get-Clipboard` 读走（base64）。要点：reset 时先短轮询归顶（≤2s），卡死才重载 c-resume iframe 再轮询（确定性回顶）；复制前会**清空系统剪贴板**（用户此时复制粘贴别的内容会被盖一下）。复制失败才退回截图 OCR。
