...
打开 Chrome，进入 BOSS直聘招聘端，登录你的招聘账号。
...
- **模型名称**：填你 AI 系统里可用的模型（实测推荐 `qwen3.5-35b-a3b_Local`）。注意避开「思考型」模型——思考型模型会一直输出思考过程，容易评不出分
...
点击 **「保存设置」**，出现绿色 **「设置已保存」** 提示且状态变为 **「✓ 已配置」** 即完成。
...
点击开始后，软件会自动检查 Chrome 连接；若 Chrome 没在运行会帮你启动。运行期间若 Chrome 被关闭或断了连接，重新打开 BOSS直聘页面后，**再点一次「开始提取分析」**即可。

---

## ⚠️ 重要：运行中的注意事项

**1. 想边用电脑边跑：给 BOSS直聘单独开一个 Chrome 窗口**

实测经验：把 BOSS直聘切到后台标签页时，模拟复制和截图都会失效、提取不到内容；**另开一个 Chrome 窗口**单独放 BOSS直聘（保持它在那个窗口的前台标签页），即使这个窗口被其他窗口完全盖住，也能正常提取。所以想一边用电脑一边跑提取，就把 BOSS直聘单独放在一个 Chrome 窗口里，日常浏览放到另一个窗口。

**2. 想边用电脑边提取：整个提取阶段别点「最小化」**

整体流程第 1 步「提取候选人」结束前，Chrome 一直在后台截图 / 读取简历，这期间别点 Chrome 右上角的「最小化」按钮——最小化会冻结页面渲染，导致截图变形、内容缺漏。想让 Chrome 让出屏幕时，用其他窗口**盖住**它即可（见第 1 条）。
...
候选人加载阶段请勿最小化 Chrome，也勿把 BOSS直聘切到后台标签页（见「运行中的注意事项」第 1、2 条）。

### 4. AI 评分失败或全部 0 分

- 确认「设置」里 API 地址 / Key / 模型填写完整，且状态为「✓ 已配置」
