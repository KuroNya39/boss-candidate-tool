...
- **邮箱信息（可选）**：填写完整邮箱地址及邮箱密码，评分结果 Excel 会自动发到你的邮箱。若邮箱开启了「三方客户端安全密码」功能，则须填写客户端安全密码，而非登录密码
...
进行步骤 1「提取候选人信息」时最小化 Chrome 会冻结页面渲染，导致页面变形、内容缺漏。想一边提取一边用电脑做别的事时，用其他窗口盖住 Chrome 即可。
...
推荐午休或下班前启动，运行期间软件会自动阻止电脑进入锁定/睡眠状态。
...
![完成界面](docs/images/图6.png)

### 分数档位

AI 会根据岗位描述里的 **3 个核心评估维度**（带权重）给候选人打分，再根据学历、经验、技能等**硬性条件**核减，最后得出 0-100 分。
