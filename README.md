...
一个帮你「从 BOSS直聘招人」的电脑小助手：它会代替你在 Chrome 浏览器里翻候选人、把每位候选人的简历交给 AI 打分、给分数达标的候选人自动打招呼，最后把所有结果整理成一张 Excel 表格发给你。
...
### 第 2 步：在 Chrome 里登录 BOSS直聘

用刚才的 Chrome 打开 BOSS直聘招聘端，登录你的招聘账号。
...
点击开始后，软件会自动将 Chrome 切换为「边用边跑」模式（被其他窗口盖住时仍正常渲染）。如果 Chrome 被重启了，重新打开 BOSS直聘页面，**再点一次开始**即可。
...
- Chrome 在普通模式运行 → 弹窗询问是否重启 Chrome（会关闭当前 Chrome 窗口）；重启后重新打开 BOSS直聘页面，再点一次「开始提取分析」即可
- Chrome 没在运行 → 自动用「边用边跑」模式启动，**直接打开你选的提取来源对应页面**（推荐牛人页/搜索页/沟通页），设好筛选条件后再点一次开始

**2. 想边用电脑边跑：别点「最小化」，用别的窗口盖住就行**

整个提取期间不要点 Chrome 右上角的「最小化」按钮。最小化会冻结页面渲染，导致截图变形、内容缺漏。

**3. 只保留一个 BOSS直聘标签页**

运行期间如需使用 BOSS直聘，请用手机端操作，避免干扰。
...
- 软件通过你自己的 Chrome 访问 BOSS直聘，不登录你的任何账号，也不修改你的任何数据
